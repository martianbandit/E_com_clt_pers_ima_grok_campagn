...
@app.route('/analyze_aliexpress_product', methods=['POST'])
def analyze_aliexpress_product():
    """Analyser un produit AliExpress avec le product manager IA"""
    try:
        aliexpress_url = request.form.get('aliexpress_url')
        
        if not aliexpress_url:
            flash('L\'URL AliExpress est obligatoire pour l\'analyse.', 'danger')
            return redirect(url_for('import_aliexpress_form'))
        
        # Créer une session temporaire pour stocker les résultats d'analyse
        if 'product_analysis' not in session:
            session['product_analysis'] = {}
        
        # Lancer l'analyse en arrière-plan
        async def process_analysis_task():
            try:
                # Initialiser le product manager
                product_manager = aliexpress_importer.AliExpressProductManager()
                
                # Analyser le produit
                analysis = await product_manager.analyze_product_url(aliexpress_url)
                
                # Générer les templates HTML
                templates = await product_manager.generate_product_templates(analysis)
                ...
                session['product_analysis'] = result
                session.modified = True
                
                return result
                
            except Exception as e:
                logging.error(f"Error analyzing AliExpress product: {e}")
                raise
        
        # Exécuter l'analyse de manière synchrone pour l'exemple
        # Dans un environnement de production, il faudrait utiliser une tâche asynchrone/background
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(process_analysis_task())
        loop.close()
        
        # Rediriger vers la page d'affichage de l'analyse
        return redirect(url_for('view_product_analysis'))
        
    except Exception as e:
        flash(f'Erreur lors de l\'analyse du produit: {str(e)}', 'danger')
        logging.error(f"Error analyzing AliExpress product: {e}")
        return redirect(url_for('import_aliexpress_form'))
        
@app.route('/view_product_analysis')
def view_product_analysis():
    """Afficher les résultats de l'analyse du product manager"""
    # Récupérer l'analyse depuis la session
    analysis = session.get('product_analysis', {})
    
    if not analysis:
        flash('Aucune analyse de produit disponible. Veuillez d\'abord analyser un produit.', 'warning')
        return redirect(url_for('import_aliexpress_form'))
